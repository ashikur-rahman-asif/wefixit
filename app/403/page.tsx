...
const ERROR_MESSAGES: Record<string, string> = {
  admin_checkout: "Admins cannot place orders or access customer dashboards. Please use a regular customer account.",
};

export default async function ForbiddenPage({ searchParams }: Props) {
  const resolvedParams = await searchParams;
  const errorCode = resolvedParams.error as string;
  const message = ERROR_MESSAGES[errorCode] || "You don't have permission to access this page. This area is restricted to administrators only.";
