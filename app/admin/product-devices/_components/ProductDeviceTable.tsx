...
interface ProductDeviceTableProps {
  ...
  isLoading: boolean;
  isDeleting: boolean;
  onEdit: (device: AdminProductDevice) => void;
  onDelete: (id: number) => void;
  onToggleStatus: (id: number, currentStatus: boolean) => void;
}

export function ProductDeviceTable({
  devices,
  pendingStatuses,
  isLoading,
  isDeleting,
            ...
            devices.map((device) => (
              ...
              >
                ...
                <TableCell className="px-6 py-4 font-semibold text-titleBlack text-sm">
                  {device.name}
                  <div className="text-xs text-textGray font-normal mt-0.5">{device.slug}</div>
                </TableCell>

                <TableCell className="px-6 py-4">
                  <Switch
                    checked={pendingStatuses[device.id] ?? Boolean(device.is_active)}
                    onCheckedChange={(checked) => onToggleStatus(device.id, checked)}
