      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        <div className="p-6 flex flex-col lg:flex-row items-center justify-between gap-4 border-b border-gray-100">
          <div className="flex w-full lg:w-auto items-center gap-4 flex-wrap">
            ...
            <Select value={status} onValueChange={(val) => handleFilterChange("status", val)}>
              <SelectTrigger className="w-full md:w-[150px] h-10 border-gray-200 text-titleBlack">
                <SelectValue placeholder="Status">
                  {status === "all" ? "All Status" : status === "pending_payment" ? "Pending" : status === "processing" ? "Processing" : status === "completed" ? "Completed" : status === "delivered" ? "Delivered" : status === "cancelled" ? "Cancelled" : "Status"}
                </SelectValue>
              </SelectTrigger>
              <SelectContent alignItemWithTrigger={false}>
                <SelectItem value="all" label="All Status">All Status</SelectItem>
                <SelectItem value="pending_payment" label="Pending">Pending</SelectItem>
            ...
            <Select value={paymentStatus} onValueChange={(val) => handleFilterChange("paymentStatus", val)}>
              <SelectTrigger className="w-full md:w-[150px] h-10 border-gray-200 text-titleBlack">
                <SelectValue placeholder="Payment">
                  {paymentStatus === "all" ? "All Payment" : paymentStatus === "pending" ? "Pending" : paymentStatus === "paid" ? "Paid" : paymentStatus === "failed" ? "Failed" : paymentStatus === "refunded" ? "Refunded" : "Payment"}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all" label="All Payment">All Payment</SelectItem>
                ...
                <SelectItem value="refunded" label="Refunded">Refunded</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
          ...
          <Table>
            <TableHeader className="bg-[#F8F9FB] border-b border-gray-100">
              <TableRow className="border-none hover:bg-transparent">
                <TableHead className="px-6 py-4 font-semibold text-titleBlack text-sm h-auto">Order Id</TableHead>
                <TableHead className="px-6 py-4 font-semibold text-titleBlack text-sm h-auto">Customer</TableHead>
                <TableHead className="px-6 py-4 font-semibold text-titleBlack text-sm h-auto">Date</TableHead>
                <TableHead className="px-6 py-4 font-semibold text-titleBlack text-sm h-auto">Items</TableHead>
                <TableHead className="px-6 py-4 font-semibold text-titleBlack text-sm h-auto">Total</TableHead>
                <TableHead className="px-6 py-4 font-semibold text-titleBlack text-sm h-auto">Payment</TableHead>
                <TableHead className="px-6 py-4 font-semibold text-titleBlack text-sm h-auto">Status</TableHead>
                <TableHead className="px-6 py-4 font-semibold text-titleBlack text-sm h-auto text-right">Action</TableHead>
              </TableRow>
            </TableHeader>
              ...
              {!isLoading &&
                orders.map((order) => (
                  ...
                  >
                    ...
                    <TableCell className="px-6 py-4 text-textGray text-sm font-semibold">{order.itemsCount} items</TableCell>
        ...
        {meta && meta.lastPage > 1 && (
          <div className="p-6 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-gray-100 text-sm text-textGray">
            <div className="flex items-center gap-3">
              <span>Show :</span>
              <Select value={perPage.toString()} onValueChange={(val) => handleFilterChange("perPage", val)}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex">
                {Array.from({ length: meta.lastPage }, (_, i) => i + 1).map((p) => {
                  if (p === 1 || p === meta.lastPage || (p >= meta.currentPage - 1 && p <= meta.currentPage + 1)) {
                    return (
                      <button
                        key={p}
                    ...
                    return (
                      <span key={p} className="w-9 h-9 flex items-center justify-center text-textGray border-t border-b border-r border-gray-200">
                        ...
                      </span>
                    );
                  }
